...
int ex1_write(struct file *file, char * buf,int count,int * ppos)
{
	...
	get_user(c,buf);
        Led_Display(9);
         mdelay(4000);  
        if(c=='1') Led_Display(1);
        if(c=='2') Led_Display(2);
        if(c=='3') Led_Display(3);
	if(c=='4') Led_Display(4);
	if(c=='5') Led_Display(5);
	if(c=='6') Led_Display(6);
	if(c=='7') Led_Display(7);
	if(c=='8') Led_Display(8);
         mdelay(4000); 
	return count;
}
...
static int __init ex1_init(void){
	int     result;
	printk("\n Registering  Device\t--- >\t");
	result = register_chrdev(DEV_MAJOR, "ex1", &ex1_fops);//ע���豸
